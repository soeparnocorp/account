...
async function handleApiRequest(path, request, env) {
  ...
  // ===== ROOM ENDPOINTS - PRIVATE ROOM CREATION DIHAPUS =====
  switch (path[0]) {
    case "room": {
      // HANYA UNTUK PUBLIC ROOM - PRIVATE ROOM CREATION TELAH DIHAPUS
      let name = path[1];
      let id;
    ...
    // ===== ENDPOINT UNTUK PRIVATE CHAT (DO UNTUK TOMBOL MULAI CHAT) =====
    case "private": {
      // Buat DO untuk chat private antar 2 user
      // Format: /api/private/userA/userB
      if (path.length >= 3) {
        const userA = path[1];
        const userB = path[2];
        ...
        // Forward request ke DO private chat
        let newUrl = new URL(request.url);
        newUrl.pathname = "/" + path.slice(3).join("/");
        return privateChat.fetch(newUrl, request);
      }
      return new Response("Invalid private chat request", {status: 400});
    }
  ...
  async webSocketMessage(webSocket, msg) {
    try {
      ...
      if (!session.name) {
        ...
        if (session.name.length > 12) { // UBAH jadi 12 karakter
...
export class PrivateChat {
  ...
  async fetch(request) {
    return await handleErrors(request, async () => {
      ...
      switch (url.pathname) {
        ...
        case "/info": {
          // Dapatkan info percakapan private
          return new Response(JSON.stringify({
            participants: this.state.id.toString().split(':')
          }));
        }
  ...
  async handlePrivateSession(webSocket) {
    ...
    // Kirim riwayat chat private
    let storage = await this.storage.list({limit: 100});
    let backlog = [...storage.values()];
    backlog.forEach(msg => {
      webSocket.send(msg);
    });
  ...
  broadcast(message) {
    ...
    this.sessions.forEach((_, webSocket) => {
      try {
