...
async function handleApiRequest(path, request, env) {
  ...
  if (path[0] === "exchange-code" && request.method === "POST") {
    try {
      ...
      // Cek user berdasarkan EMAIL di KV
      const userData = await env.READTALK_KV.get(`user:${email}`);
  ...
  // ===== ENDPOINT CEK USER (PAKE CODE LANGSUNG) - BISA DIHAPUS =====
  // if (path[0] === "check-user") { ... }
  
  // ===== ENDPOINT SIMPAN USER BARU (PAKE EMAIL) =====
  if (path[0] === "save-user" && request.method === "POST") {
    try {
      ...
      // Data user disimpan dengan KEY email
      ...
      // SIMPAN KE KV (pake email sebagai key)
      await env.READTALK_KV.put(`user:${email}`, userData);
