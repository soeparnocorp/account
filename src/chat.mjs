...
export class ChatRoom {
  constructor(state, env) {
    ...
    this.state.getWebSockets().forEach((webSocket) => {
      ...
      let limiter = new RateLimiterClient(() => this.env.limiters.get(limiterId),
  ...
  async webSocketMessage(webSocket, msg) {
    try {
      ...
      if (!session.limiter.checkLimit()) {
        webSocket.send(JSON.stringify({ error: "Your IP is being rate-limited, please try again later." }));
        return;
      }
  ...
  broadcast(message) {
    ...
    this.sessions.forEach((session, webSocket) => {
      if (session.name) {
        ...
        catch (err) { session.quit = true; quitters.push(session); this.sessions.delete(webSocket); }
      } else { session.blockedMessages.push(message); }
    });
    quitters.forEach(quitter => { if (quitter.name) this.broadcast({quit: quitter.name}); });
  }
}

export class RateLimiter {
  constructor(state, env) { this.nextAllowedTime = 0; }
  async fetch(request) {
    return await handleErrors(request, async () => {
...
class RateLimiterClient {
  ...
  async callLimiter() {
    try {
      let response;
      try { response = await this.limiter.fetch("https://dummy-url",{method:"POST"}); }
      catch(err){ this.limiter=this.getLimiterStub(); response=await this.limiter.fetch("https://dummy-url",{method:"POST"});}
